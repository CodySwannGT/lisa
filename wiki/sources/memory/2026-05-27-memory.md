...
- `project_lisa_update_workspaces_config.md` — lisa-update-projects reads its project list from .lisa.workspaces.json (not the .lisa.config.local.json the doc names); acmeorga repos live under acmeorga/projects/
...
**Impact:** During downstream updates (2026-03-19), agents had to manually restore jest configs for Expo, CDK, and some TypeScript projects (acme-product-b). This is a workaround, not a fix.
