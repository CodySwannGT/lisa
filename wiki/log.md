...
## 2026-07-26 - TASC 0.3.0 responsibility revision

- Revised `spec/tasc-0.1-draft.md` to 0.3.0-draft after a review question asked who is responsible when an autonomous agent makes a mistake. Audit of 0.2.0 found accountability was entirely technical: AC1 attributed actions to agents, `Operator` ("the human accountable for an ADS's outputs") was a definition with no criterion behind it, §6 never named a human, §9 enumerated human duties without holders, and the four accepted-risk escape hatches added in 0.2.0 (AC4.8, AC5.7, SI3, SI9) had no rule about who may use them — an agent could satisfy them by writing its own acceptance.
- Added AC1.7 (named accountable party per ADS, loop, deployment target and CHC, with backup, never an agent, legible to a non-technical reader), AC1.8 (standing to accept risk: named human, declared scope, reason and expiry; agents may neither accept nor author the acceptance), and extended AC7.2 so an agent-caused incident names the accountable party, the generation lineage, and the control that should have prevented it. Extended P5 (automation moves the work, never the accountability), §4 definitions (`Accountable party`, `Standing`), §6 (accountability register), §9 (CHC holders and backups), §11 (management's assertion by the accountable party), Annex A, and Annex D (risk-acceptance log).
- Extended the Annex B instrument (`ui/index.html`, Readiness section) from 99 to 102 questions: accountable party, standing to accept risk, and incident answerability.
- Left legal and financial liability out of scope by decision — insurance, indemnity and loss allocation sit outside what SOC 2 attests; recorded as a deliberate non-goal in the concept page's open items rather than an omission.
- Updated `wiki/concepts/tasc-specification.md` (0.3.0 revision section, CHC holders, question count) and the index Concepts entry.

## 2026-07-26 - TASC 0.4.0 standing-measurement revision

- Revised `spec/tasc-0.1-draft.md` to 0.4.0-draft after a review question asked what agents are measured against. The literal question was already settled — AC8.3 holds third-party benchmark rankings, vendor claims, and single successful runs inadmissible as qualification evidence — but the audit found TASC measured agents only at change time: `evaluation suite` appeared once, in AC8.3, as a permitted method and never as a required asset; no criterion covered agent-capability drift (the two `drift` hits were AC4.5 learning drift and AC5.2 config-mirror drift); and AC7.4 autonomy rate was the only outcome measure, so a high-autonomy factory shipping rejected work would conform.
- Added AC8.7 (evaluation suite as a maintained, representativeness-reviewed, contamination-controlled asset unreachable by the agents under evaluation), AC4.9 (capability sampled on a declared cadence against a recorded baseline, decline is a finding, vendor-side change distinguished from own accumulated change), and AC7.5 (delivery effectiveness: gate rejection, rework, escape, first-pass yield, cost per delivered item, entity-declared and ratcheted). Extended §4 (`Evaluation suite`, `Capability baseline`), §10 Level 3, §11 public summary, Annex A, Annex C, and Annex D (delivery-effectiveness statement, evaluation-suite statement with baseline history).
- Extended the Annex B instrument (`ui/index.html`, Readiness section) from 102 to 105 questions: own task suite and contamination, capability drift, and delivery effectiveness alongside autonomy rate in the bottom-line group.
- Updated `wiki/concepts/tasc-specification.md` (0.4.0 revision section, structure bullet, question count) and the index Concepts entry.

## 2026-07-26 - Anthropic AI-native SDLC ingest and TASC 0.5.0 revision

- Ingested six posts from <https://claude.com/blog> (AI-native SDLC security, Zero Trust for AI agents, verification loops in Claude Code, model selection, the Fable field guide, and context engineering for Claude 5 models), read as an adversarial review of TASC 0.4.0; wrote the reader-safe docs source note `wiki/sources/docs/2026-07-26-anthropic-ai-native-sdlc.md`.
- Revised `spec/tasc-0.1-draft.md` to 0.5.0-draft with five criteria: AC3.6 (agent-to-agent boundaries and effective authority — prompted by a documented incident in which a constrained incident-response agent asked a peer over chat to deploy for it), AC3.1's threat-to-control mapping, AC8.8 (risk-tiered autonomy), AC8.9 (staff introduction via shadow mode and adversarial exercise, with revocable trust), and AC1.9 (risk-weighted sampling of the ADS's own approvals). Added the §4 term `effective authority`, extended §6 with risk tiers and the threat-control mapping, and updated Annex A, Annex C and Annex D.
- Extended the Annex B instrument (`ui/index.html`, Readiness section) from 105 to 110 questions.
- Recorded six findings considered and deliberately deferred in the source note: SIEM routing / agents-as-insider-threat, instruction-surface coherence, dynamic-testing cadence, evaluation-suite discriminating power, implementation deviation logging, and prioritizing intake ambiguity by architectural impact.
- The posts corroborated AC4.6 (multiple narrow reviewers, findings requiring proof), SI3 (backtesting reviewers against incident history), AC3.2/AC6.5 (egress allowlisting against injection), AC6.1, AC4.5, P9, AC8.2/AC8.3 (an 80% system-prompt reduction qualified on evals), and AC8.7 (the model vendor recommending owned evaluations over saturated public benchmarks).

## 2026-07-28 - TASC 0.6.0 declared-controls and output-provenance revision

- Revised `spec/tasc-0.1-draft.md` to 0.6.0-draft after a review of 0.5.0 asked how an organization makes its policies machine-readable, how it gets visibility into every policy and control, and — separately — what catches an agent reusing proprietary or improperly attributed code. Audit confirmed all three as gaps. `policy` appeared in 0.5.0 only incidentally (retention policy, the `policy-obsolete` run outcome, DP1), so AC5.2's "same source of truth as the authoritative tier" named an artifact no criterion required anyone to have; the visibility requirement was distributed across §6's gate inventory, AC5.7's advisory inventory, AC5.3 staff parity and P9 with no single deliverable; and on licensing, AC9.3 governs declared dependencies only, so nothing reached code reproduced verbatim into the entity's own source, copied from a repository fetched during research, or carried across a client/tenant boundary by context or persisted memory — none of which has a lockfile entry or an SBOM line.
- Added AC5.8 (declared control obligations as versioned machine-readable artifacts naming enforcement tier and the independent measuring mechanism, with enforcement points and agent-facing renderings derived rather than transcribed beside them), AC5.9 (control register reconciled bidirectionally against the running system, so an obligation without an enforcement point and an enforcement point without an obligation are each findings), and SI10 (output provenance and licensing: reuse-detection gate on changed code, declared and mechanically enforced permissible-source boundary, mechanical discharge of attribution obligations that survive inlining, sources admitted to context recorded as generation lineage). Extended AC3.1 (threat class), AC5.2 (now points at AC5.8), AC9.1 (vendor reproduction filtering, attribution facilities and IP indemnity as subservice controls), AC9.3 (states its own limit), §6 (control register), and Annexes A, C and D.
- Extended the Annex B instrument (`ui/index.html`, Readiness section) from 110 to 114 questions: the reuse-detection question placed directly after the license-allowlist question whose limit it states, the permissible-source boundary with the context-surface questions, and declared obligations plus the control register in the review-and-merge group. All four left unanswered by default — an unanswered question is a known gap, not a failure.
- Corrected during review: SI10 initially routed its license-admission decision to AC9.3, whose own amendment limits it to declared dependencies. Reproduced source has no dependency record for that gate to act on, so routing the decision there applied a control to an artifact outside its boundary; admission now belongs to SI10's gate, against the same permitted-license allowlist. Annex D item 13 was also completed to carry the AC5.9 fields it had omitted (per-enforcement-point authoritative status, accountable party).
- Recorded an open follow-up rather than inventing a mechanism for it: SI10 permits regeneration as a remedy, but regenerate-from-spec (AC8.1) does not help when the same model reproduces the same memorized snippet from the same specification. A bounded retry-then-escalate rule is needed.
- Produced a three-document circulation set from the merged 0.6.0 spec — an overview/rationale brief, the full specification, and the Annex B instrument with answer choices omitted — carrying no author, company, or product identification. Build-time guards scrub and then hard-fail on any identity term; one real leak was caught that way (a harness-specific instruction filename in an intake help string, since genericized in the generator).
- Updated `wiki/concepts/tasc-specification.md` (0.6.0 revision section, enforcement-integrity and SI structure bullets, canonical-source version) and the index Concepts entry.
