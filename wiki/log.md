...
## 2026-05-28 — Stack per-agent fan-out (cursor/agy/copilot parity for stack plugins)

- Closed the parity gap where per-agent variants existed only for the base plugin: build-plugins.sh now fans out every built Claude plugin (base + 6 stacks + 2 standalones) to cursor/agy/copilot via the existing generic generators, producing `<plugin>-<agent>` variants. All 24 new variants registered in `.claude-plugin/marketplace.json`.
- `lisa apply` (processAgyEmit/processCopilotEmit) installs the base variant plus `lisa-<detected-type>-<agent>` for each detected stack (existence-filtered, so `npm-package` is skipped). agy bakes base + detected-stack eager rules into AGENTS.md (installAgyAgentsMd now accepts multiple eager dirs). Cursor needs no installer (native `.claude-plugin` consumption from the marketplace).
- Verified by run: TypeScript and Rails projects each install 2/2 variants (base + stack) on agy and copilot; per-stack shapes correct (cursor `.claude-plugin` + stripped inject-rules; agy bare manifest, no hooks; copilot `.agent.md` + camelCase hooks, inject-rules kept). Spec updated under "Stack fan-out" in `wiki/architecture/pattern-b-fan-out-spec.md`, including the eager-vs-non-eager rule-delivery nuance for agy/copilot.
