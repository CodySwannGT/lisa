...
> - **Hooks: plugin-bundled, ROOT-level.** agy DOES load a plugin's hooks from a `hooks.json` at the installed plugin ROOT (`~/.gemini/config/plugins/<variant>/hooks.json`). The earlier failure was a `hooks/` SUBDIR hooks.json, which agy does NOT scan. So `generate-agy-plugin-artifacts.mjs` emits a root `hooks.json` in agy's schema (top-level hook-name → event → handlers), matcher `run_command` (agy's shell tool) for shell-surface guards and `""` (agy's "matches all tools" spelling, compiled as a regex) for `block-direct-issue-create`, whose structured substrate arrives as `call_mcp_tool` (CodySwannGT/lisa#3785), and ships the agy-protocol script into the variant's `hooks/` subdir; the command points at the `$HOME`-absolute installed script path (agy exposes no plugin-root env var, so `$HOME` is used). Only events agy supports map (PreToolUse/PostToolUse/PreInvocation/PostInvocation/Stop) — SessionStart is unsupported, so `install-pkgs.sh`/`setup-jira-cli.sh` CANNOT ship as agy hooks; only `block-no-verify` (PreToolUse) maps. Only the BASE plugin manifest carries the universal hooks, so only `lisa-agy` gets a hooks.json (stack variants get none).
...
| `inject-resolved-config.sh` | SessionStart + SubagentStart injection of the RESOLVED Lisa configuration (`.lisa.config.local.json` merged over `.lisa.config.json`) via `additionalContext`, with built-in defaults marked, identity-shaped keys withheld, and an explicit line when no config exists. Rendering lives in the sibling `inject-resolved-config.mjs`. | Claude, Codex, Cursor, Copilot — ships wherever the agent has a session-start event, on `secrets-preflight.sh`'s reasoning rather than `inject-rules.sh`'s: Cursor strips rule injection because rules are ALSO delivered as native `.mdc`, and a resolved config has no second delivery path on any agent, so that reason does not transfer. **STRIP on agy** (no SessionStart event at all). Documented gap for agy and for OpenCode, whose session bootstrap has no context-injection surface: both are compensated at the shared rung rather than the agent layer — the gate runner refuses a config it cannot read and `lisa doctor` exits non-zero on a broken one, on every agent. |
| `enforcement-vintage.sh` | SessionStart + SubagentStart injection of WHICH COPY of Lisa is executing in this session, and whether a newer one is on the same disk. A session resolves its copy once, at session start, and runs it unchanged, so a fix merged upstream is not in effect for a session already running — measured on this fleet as an eleven-hour-old lane executing 4.32.2 while the marketplace clone sat at 4.35.1, which produced a defect filed as live that was already fixed AND a superseded contract read as current. The dispatcher's stderr notice never reaches the AGENT on the exit-0 path, so injected context is the channel. Resolution lives in the sibling `enforcement-vintage.mjs`. | Ships on `inject-resolved-config.sh`'s reasoning: the question "which copy is running" is not vendor-specific and has no second delivery path on any agent. Coverage differs BY EVENT, and the flat agent list hid that — **measured from the generated artifacts**, not from the ship rule: **SessionStart** → Claude, Codex, Cursor, Copilot; **SubagentStart** → Claude, Codex, Cursor only, because Copilot has no SubagentStart event (consistent with the Pattern B rows below and with `inject-rules.sh`). So a Copilot SUB-AGENT is never told its vintage, while a Copilot session is. **STRIP on agy** for both (no session-start event at all). Documented gap for agy, for OpenCode, and for the Copilot sub-agent case: compensated at the shared rung rather than the agent layer — the Bash enforcement dispatcher names the producing copy and its vintage in every refusal it prints, and `lisa doctor` reports the same resolution on every agent. |
...
- **Copilot (`plugins/lisa-copilot/`)** — ships `block-no-verify.sh`, `inject-rules.sh` (Copilot doesn't auto-load rules from plugin), `install-pkgs.sh`, `setup-jira-cli.sh`. Strips all SubagentStart hooks (Copilot doesn't have that event) — which includes the `enforcement-vintage.sh` SubagentStart arm, so a Copilot SUB-AGENT gets no vintage block even though the Copilot SESSION does. Strips entire calls and enforce-team-first. The Copilot variant's `inject-rules.sh` may need its `${CLAUDE_PLUGIN_ROOT}` path reference adapted if Copilot exposes a different plugin-root env var name (e.g. `${COPILOT_PLUGIN_ROOT}` — unverified; Pattern B generator should probe and document).
...
- **Marketplace delivery:** `lisa apply` writes a filtered repository catalog at
  ...
  `INSTALLED_BY_DEFAULT`. This is selection metadata, not a session-liveness
  signal: selected plugin components load only when Codex activates and trusts
  them. Current Lisa uses no user-wide plugin registration. Legacy linked hook
  directories are removed during apply, then one tagged repository enforcement
  dispatcher is reconciled into `.codex/hooks.json` (issue #3547). Project hooks
  use Codex's documented one-time `/hooks` trust flow; see the
