...
3. **Hook filter** (per the per-agent ship-list table in the audit):
   ...
   - agy: the generator emits a PLUGIN-BUNDLED root `hooks.json` (agy schema; base variant only) and ships the agy-protocol script under `hooks/`. Matcher per hook: `run_command` for the shell-surface guards, and `""` — agy's documented "matches all tools" spelling, compiled as a regex — for `block-direct-issue-create`, whose second substrate is the structured tool call `call_mcp_tool` (CodySwannGT/lisa#3785). Only `block-no-verify` (PreToolUse) is portable — agy doesn't support SessionStart, so `install-pkgs.sh`/`setup-jira-cli.sh` can't ship as agy hooks; `notify-ntfy.sh` was retired in ticket-1054. The command points at `$HOME/.gemini/config/plugins/<variant>/hooks/<script>` (the installed location). See step 5 / `emitAgyPluginHooks`.
