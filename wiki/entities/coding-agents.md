...
- Critical caveat: agy plugin-bundled hooks pass schema validation and the file installs correctly, but **registered hook handlers do not fire in `-p` headless mode**. Lisa cannot use the SessionStart-hook polyfill strategy for headless agy flows. _(An AGENTS.md rule-baking alternative was tried, then removed on 2026-06-06 — PR #1150. `AGENTS.md` is now canonical and rule-free; Lisa accepts the gap and delivers no eager-rule injection to agy.)_
