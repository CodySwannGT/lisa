 ...
 *
 * `pack` additionally proves the package records which workflow files exist at
 * the release commit. `.github/` is not in an npm files allowlist, so an
 * installed package cannot otherwise learn it; a consumer's pinner reads that
 * stamp to leave a caller alone when the release does not carry its callee,
 * and an unstamped or drifted inventory silently restores the rewrite that
 * produces an unloadable workflow.
...
/**
 * Where a repository's workflow files live.
 *
 * The inventory of this directory at the release commit is stamped into the
 * package because `.github/` is not in an npm files allowlist and so does not
 * travel with the tarball. A consumer's reusable-workflow pinner reads it to
 * refuse to rewrite a caller whose callee is not in the release: a pin at a
 * commit the workflow is absent from does not fail, it never loads, so the run
 * creates zero jobs, reports zero failures, and names no missing file
 * (CodySwannGT/lisa#4021).
 */
const WORKFLOW_DIR = path.join(".github", "workflows");

/** Extensions GitHub Actions recognizes for a workflow file. */
const WORKFLOW_FILE = /\.ya?ml$/u;

/** The manifest read from the release checkout, and its diagnostic label. */
...
/**
 * List the workflow files present at the release commit.
 *
 * Sorted so the stamp is a function of the tree and not of directory order,
 * which is what lets the packed candidate be compared against it byte for
 * byte. An absent directory answers an empty list rather than failing: a
 * repository that ships no workflows has nothing to stamp, and refusing to
 * publish it would be a new failure rather than a closed one.
 * @param {string} root Repository root
 * @returns {readonly string[]} Workflow file names, sorted
 */
export function releaseWorkflowInventory(root) {
...
/** Require a stamped workflow inventory that matches the release tree. */
function assertWorkflowInventory(stamped, expected, label) {
  if (!Array.isArray(stamped)) {
    mismatch(
      `${label} lisaReleaseWorkflows ${JSON.stringify(stamped)} is not an array; ` +
        `the release commit carries ${expected.length} workflow file(s), and a consumer that cannot read this ` +
        "inventory pins callers at commits their callee may not exist in"
    );
  }
  const actual = JSON.stringify(stamped);
  if (actual !== JSON.stringify(expected)) {
    mismatch(
      `${label} lisaReleaseWorkflows ${actual} does not match the release tree ${JSON.stringify(expected)}`
    );
  }
}
 ...
 * @returns {{version: string, releaseCommit: string, releaseTag: string, releaseWorkflows: readonly string[], certificateVersion: string|null, tarballPath: string, tarballSha256: string, certificateMemberSha256: string|null}}
 */
export function packAndValidateReleaseCandidate({
  ...
  const packageJson = readJson(path.join(root, PACKAGE_JSON), PACKAGE_JSON);
  ...
  const workflowInventory = releaseWorkflowInventory(root);
  if (workflowInventory.length > 0) {
    assertWorkflowInventory(
      packageJson.lisaReleaseWorkflows,
      workflowInventory,
      PACKAGE_JSON
    );
  }
  ...
  if (workflowInventory.length > 0) {
    assertWorkflowInventory(
      packedPackage.lisaReleaseWorkflows,
      workflowInventory,
      "packed package"
    );
  }
  ...
  return Object.freeze({
    ...
    releaseWorkflows: workflowInventory,
...
export function runReleasePackageIdentityCli(
  ...
  if (mode === "workflows") {
    // Printed rather than stamped here so the publishing workflow does the
    // writing and this file stays the single reader of the release tree: the
    // stamp and the check that validates it cannot describe different
    // directories if only one function ever lists it.
    process.stdout.write(`${JSON.stringify(releaseWorkflowInventory(root))}\n`);
    return 0;
  }
  mismatch("expected mode checkout, pack, or workflows");
}
