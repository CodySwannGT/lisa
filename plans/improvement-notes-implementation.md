...
| — | acme-product-b v3 bump (owner-approved; logoman deliberately left alone) | in flight |
...
acme-product-b bump. (a) `quality.yml`'s `work_item_traceability` has no `permissions:`
...
2. ✅ **The fleet has ~2 copy-propagated lineages, not 14 independent files.** Frontend blob: acmeorgd/frontend ≈ acmeorgc/frontend ≈ logoman (identical headings down to the same expo issue numbers). Backend blob: `acme-product-b` is **40 of 42 lines identical** to `acmeorga/backend` — it carries acmeorga's "pennies vs cents" *monetary* rule inside a soccer scouting product. **Routing each lineage once fixes 2–3 repos**, which changes the shape of the remaining ladder work.
3. ✅ `acme-product-b` (49 lines) and `infrastructure-v2` (64 lines) were mis-surveyed as absent/empty.
