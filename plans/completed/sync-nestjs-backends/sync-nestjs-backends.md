...
1. **Create branch and run Lisa sync on acmeorgb/backend-v2** — `cd /Users/cody/workspace/acmeorgb/backend-v2 && git checkout -b chore/lisa-sync-v1.15 dev`, then `cd /Users/cody/workspace/lisa && bun run dev /Users/cody/workspace/acmeorgb/backend-v2 -y`
2. **Migrate acmeorgb jest config to modular structure** — Move project-specific exclusions from old `jest.config.ts` into `jest.config.local.ts`, set `jest.thresholds.json` if needed, verify coverage exclusions match original behavior
