...
Running `bun run knip` in acmeorga/frontend after Lisa update surfaces two issues:
...
### 2. acmeorga/frontend: Add graphql path mapping to tsconfig.local.json

**File**: `/Users/cody/workspace/acmeorga/frontend/tsconfig.local.json`
...
2. `bun run typecheck` in acmeorga/frontend — passes
3. `bun run knip` in acmeorga/frontend — no `serve` or `@/graphql/graphql` errors

## Commits

1. **Lisa repo**: `fix: add serve to expo knip ignoreBinaries for ZAP workflow`
2. **acmeorga/frontend**: `fix: add graphql path mapping to tsconfig.local.json for knip resolution`
