...
1. Reads `vitest.thresholds.json` (if present) or falls back to `jest.thresholds.json` to get current coverage thresholds
...
4. Updates `vitest.thresholds.json` (or `jest.thresholds.json` if that was the source) with the new values
...
`vitest.thresholds.json` (or `jest.thresholds.json`) format:
```json
{
  "global": {
    "statements": 75,
