...
export function assertCheckoutIdentity({ root, releaseCommit, tag }) {
  assertReleaseCommit(releaseCommit);
  const headCommit = gitCommit(root, "HEAD");
  if (headCommit !== releaseCommit) {
    mismatch(
      `HEAD ${headCommit} does not match release commit ${releaseCommit}`
    );
  }
  const tagCommit = gitCommit(root, tag);
  if (tagCommit !== releaseCommit) {
    mismatch(`tag ${tag} resolves ${tagCommit}, expected ${releaseCommit}`);
  }
  return Object.freeze({ headCommit, tagCommit, releaseCommit });
}
...
export function packAndValidateReleaseCandidate({
  ...
  if (packageJson.lisaReleaseCommit !== releaseCommit) {
    mismatch(
      `package.json lisaReleaseCommit ${JSON.stringify(packageJson.lisaReleaseCommit)} expected ${releaseCommit}`
    );
  }
  if (packageJson.gitHead !== releaseCommit) {
    mismatch(
      `package.json gitHead ${JSON.stringify(packageJson.gitHead)} expected ${releaseCommit}`
    );
  }
  ...
  if (packedPackage.lisaReleaseCommit !== releaseCommit) {
    mismatch(
      `packed lisaReleaseCommit ${JSON.stringify(packedPackage.lisaReleaseCommit)} expected ${releaseCommit}`
    );
  }
  if (packedPackage.gitHead !== releaseCommit) {
    mismatch(
      `packed gitHead ${JSON.stringify(packedPackage.gitHead)} expected ${releaseCommit}`
    );
  }
  ...
  if (hasCertificate) {
    ...
    if (packedCertificateVersion !== version) {
      mismatch(
        `packed certificate names ${packedCertificateVersion}, expected ${version} at release commit ${releaseCommit} (${PACKED_CERTIFICATE})`
      );
    }
