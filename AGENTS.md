...
**The goal of the factory setup is to let non-technical people create scalable, enterprise-grade software** by describing outcomes — the factories supply the engineering discipline. That lowers the floor without capping the ceiling: engineers get leverage from the same machinery, but technical skill stops being the entry requirement. It also creates a communication obligation: everything that crosses a gate outward — blocked reasons, clarifying questions, ticket descriptions, verification reports — must be readable by a non-technical operator, because that is who is standing at the gate.
