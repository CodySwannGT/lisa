...
This command compares Lisa's source templates against a target project that has Lisa applied, identifying what the project has changed from Lisa and offering to adopt those improvements back into Lisa.

This is the inverse of `/lisa:review-implementation`:
- **review-implementation**: Run FROM project → "What did I change from Lisa?"
- **review-project**: Run FROM Lisa → "What has this project changed?"

## Prerequisites

This command must be run FROM the Lisa repository directory. The target project must have Lisa already applied (must have a `.lisa-manifest` file).

## Instructions

### Step 1: Validate Lisa Directory

Confirm we're running from Lisa by checking for `src/core/lisa.ts`.
...
**Example:**
```
Adopting improvements from test-project...

✓ Copied .github/workflows/ci.yml to typescript/copy-overwrite/.github/workflows/ci.yml
✓ Copied eslint.config.mjs to typescript/copy-overwrite/eslint.config.mjs

Next steps:
1. Review the changes in Lisa
2. Test the updated templates on another project
3. Commit to Lisa repo
```
...
```
User: /lisa:review-project ~/workspace/my-expo-app

Claude: I'll compare Lisa's templates against your Expo project.

[Validates Lisa directory]
[Validates project and reads manifest]
[Detects types: typescript, expo]
[Compares 45 managed files]

# Lisa Project Review

**Lisa Directory:** /Users/dev/lisa
**Target Project:** /Users/dev/workspace/my-expo-app
**Project Types:** typescript, expo
...
**Analysis:** Project added pnpm caching to speed up CI. This improves performance for all projects using this template.

---

I found 2 files that differ from Lisa's templates.

Would you like to:
1. Adopt improvements (recommended)
2. View full report with all details
3. Done - no changes

Select: [user selects 1]

Great! These files are candidates for adoption:
- .github/workflows/ci.yml (Improvement)
- eslint.config.mjs (Customization)

Which would you like to adopt? (or 'all'):
```
