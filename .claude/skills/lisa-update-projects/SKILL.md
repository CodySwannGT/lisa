...
8. Check for legacy inline Claude workflows that need migration. For each file in `.github/workflows/` matching `claude*.yml`, `claude*.yaml`, `auto-update-pr-branches.yml`, `auto-update-pr-branches.yaml`, `ci.yml`, `ci.yaml`, `deploy.yml`, and `deploy.yaml`:
   ...
   - Additionally, check all workflow files for `uses: ./.github/workflows/` references that point to local copies of Lisa reusable workflows (e.g., `quality.yml`, `release.yml`, `load-test.yml`, `create-issue-on-failure.yml`, `create-*-on-failure.yml`). These local copies are deleted by Lisa's `deletions.json`, so any remaining references will break. Rewrite them to `uses: CodySwannGT/lisa/.github/workflows/<name>.yml@main`. When converting, `secrets: inherit` is not supported with external workflow references — replace it with explicit `secrets:` blocks that pass each required secret individually. Check the Lisa reusable workflow's `workflow_call.secrets` definition to determine which secrets to pass.
