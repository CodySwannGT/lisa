...
4. Transition via `lisa-linear-access operation: save-issue lifecycle_role: done env: <resolved-env-key>` (from `$CLAIMED`, or from `$REVIEW` if `lisa-linear-evidence` already moved it forward). `done` is env-indexed, so the environment key is mandatory — the access layer refuses a bare `done` and resolves `$DONE` itself rather than accepting the ID this phase computed.
