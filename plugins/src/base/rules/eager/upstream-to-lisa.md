...
2. Search open and closed Lisa issues, including prior **Not planned** decisions. Reuse an existing relevant issue or file one upstream with the concrete consequence and smallest durable fix. Follow `rejection-detection` for declines; do not refile an accepted limitation.
