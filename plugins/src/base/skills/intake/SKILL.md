...
4. **Process the first eligible Ready item only** (claim-first ordering for idempotency):
   ...
   - **Closing the PRD loop:** beyond claiming one Ready PRD, every PRD scanner also runs the closure rollup (`ticketed → shipped`, Phase 3f) and **dispatches `lisa:verify-prd` for one shipped PRD** (`shipped → verified` on pass, `shipped → blocked` on fail, Phase 3g) each cycle — so a shipped PRD does not sit unverified. The scanner only dispatches; `lisa:verify-prd` owns the transition (per the `prd-lifecycle-rollup` rule's "Closing the loop" section).
