...
- **The terminal value must have survived the promotion-completeness gate** (`config-resolution` → "Promotion-completeness gate"). Reaching the production branch is not the same as having been promoted through the environments below it: a fix merged straight to `main` while `staging` lacks the merge commit resolves to an intermediate env under the gate, and an intermediate value never closes anything. Native closure is authorized by a *gated* terminal value, never by a raw base-branch reverse-lookup. A promotion gap is incomplete delivery, not branch hygiene — an open back-fill PR against the skipped environment holds the item open rather than excusing it.
