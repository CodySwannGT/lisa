...
- Ticket descriptions MUST contain everything an implementer needs to start work without asking the reporter. In particular:
  - **Target backend environment** (`dev` / `staging` / `prod`) when the ticket has runtime behavior. QA/product report against a deployed env; the implementer verifies locally first against that backend before CI/CD. Skip only for doc-only, config-only, or type-only tickets.
