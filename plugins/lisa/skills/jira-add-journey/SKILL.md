...
7. **On a leaf work unit with `runtime_behavior_change = true`, the markers are binding** — For a Bug / Task / Sub-task / Improvement where `runtime_behavior_change = true`, every `[EVIDENCE: name]` here is the ticket's evidence manifest: validation gate S14 requires at least one, and the ticket cannot be closed until each named artifact is captured and attached (see the "Per-Work-Unit Evidence Contract" in the `verification` rule). Name only evidence you intend to capture — and name all of it. (For doc-only / config-only / type-only leaf units with `runtime_behavior_change = false`, S14 is N/A and markers are not required.)
