...
- `target_branch=<branch>` or `base=<branch>` — intended PR base branch. Still required for base resolution and for the Linear magic-word decision; it is no longer consulted for GitHub, whose reference is non-closing on every branch.
...
- **GitHub Issues**:
  ...
  - Always use a **non-closing** reference such as `Refs #<n>`, on every base branch including the repository default and configured production branch. `Refs` still populates the issue's Development / linked pull requests surface, which is the linkage `lisa-github-sync` reads.
  - **Never** emit a GitHub closing keyword (`Closes`/`Fixes`/`Resolves #<n>`). Merging to the production branch is not terminal delivery: `lisa-implement` places deploy monitoring, remote verification, and a post-deploy health check *after* the merge, and the work item is terminal only on a passing remote verdict. A closing keyword closes the issue at merge — before the terminal `done` role is applied and before anything has verified the change ran — which front-runs that gate. Closure belongs to the `leaf-only-lifecycle` "Terminal native closure" invariant (`gh issue close --reason completed`, after `done`) — cite it, do not restate. This is the same premature-closure failure mode already forbidden for Linear below; GitHub differs only in blast radius, not in kind.
  - For cross-repo issue refs, use the fully qualified form, for example `Refs CodySwannGT/lisa#614`.
- **Linear**:
