...
Re-query the PRDs currently carrying `$SHIPPED` via `gh issue list --repo <org>/<repo> --label "$SHIPPED" --state all`. Using `--state all` is required when `github.labels.prd.rollup.closeOnShipped = true` causes the PRD issue to be closed on ship — `--state open` would permanently skip those PRDs from verification. Pick the **first** one and invoke `lisa:verify-prd <issue-url>`. Process **at most one shipped PRD per cycle** (in addition to the at most one ready PRD processed in Phase 3) — `lisa:verify-prd` is a heavy full flow (spec-conformance + empirical verification + fix-issue creation), so it is bounded exactly like the single-ready-PRD claim in Phase 3; the scheduler drains the rest. Each cycle dispatches at most one ready PRD (Phase 3) and at most one shipped PRD (Phase 3g), for a combined maximum of two PRD operations per cycle.

`lisa:verify-prd` owns the outcome: on a CONFORMS verdict with all empirical checks passing it transitions `$SHIPPED → verified` and posts evidence; on a conformance miss or a failing/unavailable check it **re-opens the PRD `$SHIPPED → ticketed`** (never `blocked`) and creates **build-ready** fix tickets registered as the PRD's generated work, then posts a failure report — the fix tickets auto-build, rollup (3f) re-ships the PRD once they are terminal, and a later cycle re-verifies (the self-healing loop). Either branch moves the PRD out of `$SHIPPED`, so it is not re-picked this cycle; a PRD whose generated work is not actually terminal is guard-stopped by `lisa:verify-prd` (left `$SHIPPED`) — that is verify-prd's gate, not this skill's. A PRD closed on ship (`github.labels.prd.rollup.closeOnShipped = true`) retains the `$SHIPPED` label and is included via `--state all` so it still reaches `lisa:verify-prd`. This phase, like 3f, is **behaviorally identical across all four intake skills** (`github-prd-intake`, `linear-prd-intake`, `notion-prd-intake`, `confluence-prd-intake`) — only the `$SHIPPED` query surface differs; keep them aligned. Record the dispatched PRD + verify-prd's verdict in the summary.
