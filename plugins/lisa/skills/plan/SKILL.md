...
| A Linear **project** URL (`https://linear.app/<workspace>/project/<slug>-<id>`) used as a **PRD source** (convert into tracker tickets) | `lisa:linear-to-tracker` (with the project URL; same full pipeline as the Notion / Confluence paths). The Linear project's description, attached documents, and sub-issues form the PRD body. This branch takes precedence unless the destination tracker is `linear` AND the intent is to decompose the project as a build-side artifact (see next applicable row). |
...
| A Linear Project URL that is already a **build-side tracker artifact** (not a PRD source), when destination tracker = `linear` | `lisa:linear-agent` (read project, decompose into Issues / sub-Issues). Symmetric counterpart to the JIRA-Epic and GitHub-Epic branches. This branch only applies when the user explicitly intends the project as the spec-to-implement, not as PRD intake. |
