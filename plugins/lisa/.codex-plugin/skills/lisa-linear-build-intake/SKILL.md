...
**Claim-time archaeology runs second — after rejection detection, still before the relabel below.** Per the vendor-neutral `claim-archaeology` rule (cite the slug; do not restate its signal bindings or budget semantics), classify this Issue `fresh | retry-of-done-issue | rejection-reclaim`. The rejection classification above is the input: `rejection-reclaim` maps straight through (reused, never re-derived; its reflection above already carries the learning). Otherwise search ancestry — the native relations already in the read bundle, `lisa-linear-access operation: list-issues` filtered to recently-closed Issues, deterministic git ancestry (`{file, sha, pr, date}`) — inside the configured `archaeology.maxSteps` budget (`jq -r '.archaeology.maxSteps // 8' .lisa.config.json`). On `retry-of-done-issue`, derive ONE delta-citing candidate from the ancestor's PR/review threads/evidence comments and route it to `lisa-persist-learning`; if absent, record it via `lisa-linear-access operation: save-comment` with the visible prose line then `<!-- [lisa-archaeology-candidate] key=<issue>-<ancestor> -->` (marker dedupe — re-claims produce no duplicate). `fresh` → no candidate, zero comments. Budget exhausted, weak signals, or an error → degrade to `fresh` and continue: archaeology never blocks the claim.
