...
6. **"Artifact identity" and "Not established" sections (both required).** Right after "What this shows":
   - `## Artifact identity` — what the evidence was captured against: repository, head SHA, environment, capture time (values pinned by BCE-4; render the heading with what is known).
