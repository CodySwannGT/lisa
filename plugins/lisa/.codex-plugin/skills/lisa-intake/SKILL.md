...
- A GitHub URL or `owner/repo` token is an explicit queue and always wins. The literal token
  `github` (case-insensitive), or an omitted repo when config selects GitHub, resolves local then
  global `github.queueRepo`, then identity `github.org/github.repo` **for build dispatch**. Normalize
  a short queueRepo to `github.org`. PRD dispatch remains on the configured GitHub PRD source. Do
  not use the build queue repo as current-repo identity for `repo:<name>` scoping.
