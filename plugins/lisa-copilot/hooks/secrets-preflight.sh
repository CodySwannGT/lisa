...
# An escape hatch that is honest about what it costs. Set when a surface has no
# provider by design and the noise is not worth it; the doctor and CI gates are
# unaffected, so opting out here does not opt out of the check entirely.
if [ "${LISA_SKIP_SECRETS_PREFLIGHT:-}" = "1" ]; then
  exit 0
fi
