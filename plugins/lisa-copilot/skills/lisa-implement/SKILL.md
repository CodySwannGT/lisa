...
**One roster file per work item — never one shared roster file per repository.** `<work-item-slug>` is the canonical `work_item_ref` this flow is bound to, with every character outside `[A-Za-z0-9._-]` replaced by `-`, so two runtimes working the same item derive the same filename and two flows working different items in one repository never write the same path. It follows the same per-flow convention as the `.lisa/plan-<id>.md` artifacts Lisa already writes under `.lisa/`. A single shared `.lisa/roster.md` is the defect: the first time each branch creates it the merge is **add/add**, which git cannot three-way, so resolution is a coin flip that silently discards one flow's roster — and because the file reads as scratch, that discard looks harmless while the surviving roster belongs to a different flow than the one being audited. Do not migrate or delete a legacy `.lisa/roster.md` a repository already tracks; leave it in place and write the per-work-item file alongside it. The roster remains a tracked audit record either way — do not reclassify it as runtime scratch or add it to the ignore set.
