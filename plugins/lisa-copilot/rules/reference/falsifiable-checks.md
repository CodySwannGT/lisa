...
A test asserting that the **correct** marker is *present* passes trivially when the file happens to state it twice — the second statement satisfies the assertion no matter what the first one says, so the test survives the exact edit it exists to catch. The falsifiable form asserts the **wrong** spelling is *absent*: that assertion has no second copy to fall back on, and it goes red the moment the wrong form reappears. Where both are cheap, write both, and mutation-prove the pair.
