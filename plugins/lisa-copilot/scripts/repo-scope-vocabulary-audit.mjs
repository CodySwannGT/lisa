 ...
 * ## The gap this closes
 *
 * `/lisa:validate-tracker-mapping` audits LIFECYCLE roles: every pair it
 * compares comes from a `(role, configured-name)` mapping declared in
 * `.lisa.config.json`. The SCOPING vocabulary is declared in no config key, so
 * the audit has nothing to compare it against.
 *
 * `repo:` is the scoping family where that blind spot costs the most, because
 * a wrong answer there is not a matter of taste: it decides which work items a
 * repo's build queue can see. When the repo vocabulary drifts, a scan
 * filtering on the canonical spelling returns FEWER items, not an error, and
 * an empty result is indistinguishable from "nothing to do". Work sits
 * unrouted and nobody is told.
 *
 * The OPEN scoping families — `type:`, `priority:`, `points:`, `component:` —
 * are a separate problem with a separate answer, tracked upstream on #3420.
 * This module deliberately does not touch them, for the reason below.
 *
 ...
 *
 * The open families need a heuristic — is `component:plugin` a typo of
 * `component:plugins`? — and a heuristic is sometimes wrong, which is why
 * #3420 rules that findings about them must stay advisory and must never
 * become a gate. That constraint exists to stop a GUESS becoming a gate, and
 * it is not weakened here, because nothing here is a guess. `assertRepoScope`
