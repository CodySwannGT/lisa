...
export function reviewGateVerdict(reading = {}) {
  ...
  if (waived.length > 0) {
    ...
    return {
      ...
      title: fitTitle(
        `${escalated ? "WAIVED, AND THE WAIVER RATE HAS PASSED THE THRESHOLD THIS REPOSITORY SET" : "WAIVED"} — this pull request is UNREVIEWED and merging it is a decision taken on that basis: ${waived.map(quote).join("; ")}${suffix}`
      ),
    };
  }
