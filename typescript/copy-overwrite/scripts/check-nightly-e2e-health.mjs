...
export function validateSuites(raw) {
  ...
  const suites = parsed.map((entry, index) => {
    ...
    // Rows 27-30 — the per-suite grace anchor and its length. Shape only; the
...
/**
 * Resolves ONE suite's first-seen grace window (rows 27-30).
 *
 ...
 * did something.
 *
 * A finding may carry its OWN window in `finding.grace` (rows 27-30), resolved
