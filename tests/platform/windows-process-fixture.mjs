 ...
 */
export function fixtureProcess(pid, childPath, stop = false) {
 ...
 * @param {boolean} holdShell Whether to retain the direct shell for timeout tests.
 * @returns {object} Paths and the shell command for this fixture.
 */
export function fixture(t, holdShell = false) {
 ...
 * @param {number} expectedCode Expected supervisor exit code.
 * @returns {boolean} Whether the uniquely identified descendant remains alive.
 */
export function observe(f, result, expectedCode = 0) {
 ...
 */
export function runSupervisor(f) {
  ...
  try {
    const result = spawnSync(
      process.execPath,
      [supervisor, SUPERVISOR_DEADLINE, "--", f.command],
