...
describe("lisa worktree prune against real git", () => {
  it("removes a quiescent worktree whose branch is fully published", async () => {
  ...
  it("does NOT remove a sibling worktree a live process is working inside", async () => {
    const root = await createPublishedRepo();
  ...
  it("does NOT discard a worktree holding unpushed commits and uncommitted edits", async () => {
    const root = await createPublishedRepo();
    ...
    expect(runGit(["log", "-1", "--format=%s"], atRisk)).toBe("unpushed work");
  }, 120_000);

  it("never removes the primary checkout or the worktree it runs from", async () => {
    const root = await createPublishedRepo();
...
describe("lisa stash prune against real git", () => {
  it("drops aged machine debris, keeps real work, and preserves what it drops", async () => {
    const root = await createPublishedRepo();
...
describe("the CLI runners", () => {
  it("removes nothing when --apply is absent", async () => {
    const root = await createPublishedRepo();
    const finished = addPublishedWorktree(root, "finished");
  ...
  it("lets a claim make a fresh worktree of the caller's own eligible", async () => {
    const root = await createPublishedRepo();
  ...
  it("writes the claim inside the control plane, leaving the tree clean", async () => {
    const root = await createPublishedRepo();
    const mine = addPublishedWorktree(root, "mine");
