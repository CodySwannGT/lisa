...
describe('MergeStrategy', () => {
  ...
  it('copies file when destination does not exist', async () => {
    ...
    const result = await strategy.apply(srcFile, destFile, 'package.json', createContext());
  ...
  it('does not modify files in dry run mode', async () => {
    ...
    const result = await strategy.apply(srcFile, destFile, 'package.json', createContext({ dryRun: true }));
  ...
  it('throws error for invalid source JSON', async () => {
    ...
    await expect(strategy.apply(srcFile, destFile, 'invalid.json', createContext())).rejects.toThrow();
  });
