...
describe('CreateOnlyStrategy', () => {
  ...
  it('creates file when destination does not exist', async () => {
    ...
    const result = await strategy.apply(srcFile, destFile, 'README.md', createContext());
  ...
  it('does not modify files in dry run mode', async () => {
    ...
    const result = await strategy.apply(srcFile, destFile, 'README.md', createContext({ dryRun: true }));
