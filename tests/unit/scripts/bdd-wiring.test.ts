...
describe("shipped wiring", () => {
  ...
  it("prints executable base-aware package commands in both generated document preambles", () => {
    const matrixGenerator = read(MATRIX_REL);
    const burndownRenderer = read("expo/copy-overwrite/scripts/bdd/render.mjs");
    const matrixCommand =
      'BDD_BASE_SHA="$(git merge-base HEAD origin/HEAD)" bun run bdd:matrix';
    const burndownCommand =
      'BDD_BASE_SHA="$(git merge-base HEAD origin/HEAD)" bun run bdd:coverage:write';

    expect(matrixGenerator).toContain(matrixCommand);
    expect(matrixGenerator).not.toContain(
      "node scripts/bdd-matrix.mjs --write"
    );
    expect(burndownRenderer).toContain(burndownCommand);
    expect(burndownRenderer).not.toContain(
      "node scripts/check-bdd-coverage.mjs --write"
    );
    const matrix = read("docs/bdd-scenario-matrix.md");
    const burndown = read("docs/e2e-bdd-coverage.md");
    expect(matrix).toContain(`Generated by \`${matrixCommand}\``);
    expect(burndown).toContain(`Regenerated by \`${burndownCommand}\``);
    expect(matrix.startsWith("# BDD scenario traceability matrix\n")).toBe(
      true
    );
    expect(
