...
function virtualTree(directoryCount: number, fileCount: number): VirtualTree {
  ...
  return {
    fs: {
      ...
      // A direct-child rename that carries the whole subtree with it. The
      // cleanup program quarantines a child under an unguessable name and
      // scans it there, so a tree that cannot be renamed cannot be scanned.
      renameSync: ((from: string, to: string) => {
        const remap = (candidate: string): string =>
          candidate === from ? to : `${to}${candidate.slice(from.length)}`;
        const moves = [...directories, ...files, ...children.keys()].filter(
          candidate => candidate === from || candidate.startsWith(`${from}/`)
        );
        for (const candidate of new Set(moves)) {
          const target = remap(candidate);
          if (directories.delete(candidate)) directories.add(target);
          if (files.delete(candidate)) files.add(target);
          const entries = children.get(candidate);
          if (entries !== undefined) {
            children.delete(candidate);
            children.set(target, entries);
          }
        }
        const [parent, basename] = parentOf(from);
        children.get(parent)?.delete(basename);
        children.get(parent)?.add(path.posix.basename(to));
        renameCount += 1;
      }) as (...args: never[]) => unknown,
    },
...
function runCleanup(tree: VirtualTree): void {
  ...
  runInNewContext(BOUND_DIRECTORY_CLEANUP_PROGRAM, {
    ...
    process: {
      argv: ["node", "1", "2"],
      // The program reads fault-injection switches off `process.env`. An empty
      // object keeps every one of them off, which is what a budget case wants,
      // and keeps the read from throwing on an absent `env`.
      env: {},
      exit: (code: number) => {
...
describe("scratch cleanup entry budget", () => {
  ...
  it("bounds a selected child tree without deleting any of it", () => {
    const tree = virtualTree(1, 100_001);

    expect(() => runChildCleanupPreflight(tree)).toThrow(/entry bound/iu);
    // The scan happens AFTER the quarantine rename, deliberately: a pre-rename
    // scan leaves a window for a writer to create an entry the scan never
    // records, which then fails the final rmdir and strands the whole tree. So
    // the guarantee this case holds is not "no rename" but "no deletion" — the
    // over-cap tree is refused with every one of its entries still present.
    expect(tree.renames()).toBe(1);
