...
/**
 * The unrecognised detail an inspection carries for entries with no marker.
...
describe("inspectNamespace", () => {
  ...
  it("classifies every markerless root as unowned authority", () => {
    ...
    expect(residue).toEqual({
      ...
      unrecognisedDetail: absentDetail(FOREIGN_ENTRY, DEAD_ROOT, LIVE_ROOT),
      total: 3,
    });
  });
  ...
  it("classifies a corrupt marker on a recognized name as unowned", () => {
    ...
    expect(inspectNamespace(dir, () => true)).toEqual({
      ...
      unrecognisedDetail: [{ name: LIVE_ROOT, reason: "marker-unreadable" }],
      total: 1,
    });
  });
