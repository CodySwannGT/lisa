...
1. **Declared** — behaviors written as Gherkin scenarios. 6 total, 6 in the denominator, 0 excluded (0 blocked, 0 reference-only, 0 superseded).
...
**No execution evidence was supplied to this run.** 6 mapped tests exist; how many ran, and what they returned, is unknown here. Pass `--results <file>` with a runner result document to populate this section. Traceability without execution proves automation was written, never that it works.
...
61 tests discovered under `tests/e2e`; 61 are named by a mapping or an exclusion. 1 carry a computed (template-literal) title, taken verbatim from the source — a runner result cannot be joined to those by title.
...
6 of 6 scenarios carry a tracker tag; 0 do not. Tags are validated for syntax and against this repo's declared trackers; the gate never contacts a tracker, so an unreachable issue can never block a merge.
