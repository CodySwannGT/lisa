...
Feature: Starter sync results in the Lisa console
  ...
  Scenario: Sync reports the engine result and preserves failures
    Given the live console was opened through its private launch link
    And it can invoke its project-bound starter sync engine
    ...
    And reloading requires the launch link again before another sync
